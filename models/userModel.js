...
const UserSchema = new Schema(
  {
    ...
    phone: {
      ...
      validate(value) {
        if (!validator.isMobilePhone(value)) throw new Error("Number is invalid");
      },
    },
...
UserSchema.statics.alreadyLogin = async function (phone) {
  const user = await User.findOne({ phone });
  if (user) throw new Error("Already phone registered");
};
